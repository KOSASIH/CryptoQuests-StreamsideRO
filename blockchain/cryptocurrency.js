class Cryptocurrency {
  ...
  minePendingTransactions(miningRewardAddress) {
    let block = new Block(this.blockchain.length, this.blockchain[this.blockchain.length - 1].hash, Date.now(), this.pendingTransactions);
    block.mineBlock(this.difficulty);
    this.blockchain.push(block);
    this.pendingTransactions = [new Transaction(null, miningRewardAddress, this.blockReward)];
  }
  ...
  isChainValid() {
    for (let i = 1; i < this.blockchain.length; i++) {
      ...
      if (currentBlock.hash !== this.generateHash(currentBlock.index, previousBlock.hash, currentBlock.timestamp, currentBlock.transactions, currentBlock.nonce)) {
        return false;
      }
    }
    return true;
...
class Block {
  ...
  mineBlock(difficulty) {
    while (this.hash.substring(0, difficulty) !== Array(difficulty + 1).join("0")) {
      this.nonce++;
      this.hash = Cryptocurrency.generateHash(this.index, this.previousHash, this.timestamp, this.transactions, this.nonce);
    }
  }
}
